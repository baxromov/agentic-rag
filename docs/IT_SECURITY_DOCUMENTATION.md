...
| Порт | 5173 |
| Назначение | Веб-интерфейс пользователя (React SPA) |
| Зависимости | FastAPI Backend |
| Volumes | Исходный код frontend монтируется для hot-reload |

### 3.4. Model Server (Reranker)
...
Отдельный инстанс PostgreSQL для Langfuse описан в разделе 3.10.
...
### 3.9. MinIO (S3-совместимое хранилище)
...
### 3.10. Langfuse v3 (Observability) — опциональный
...
### 3.11. Ollama (на хост-машине, вне Docker)
...
| Модели | LLM (напр. `llama3.1`) + Embedding (`nomic-embed-text:latest`) |
...
Для модели **llama3.1:8b** (текущая конфигурация по умолчанию):

| Квантизация | Требуемая VRAM | Пример конфигурации GPU |
|-------------|---------------|------------------------|
| Q4 (по умолчанию в Ollama) | ~5–6 GB | 1× NVIDIA RTX 3060 12GB |
| Q8 | ~9–10 GB | 1× NVIDIA RTX 3080 10GB |
...
#### Минимальная (с llama3.1:8b Q4, без Langfuse)

| Ресурс | Значение |
|--------|---------|
| CPU | 16 ядер |
| RAM | 32 GB |
| GPU | 1× NVIDIA с 12+ GB VRAM (RTX 3060/4060 и выше) |
| Диск | 200 GB SSD |
...
| Frontend (Vite) | 1 ядро | 512 MB | 1 GB | 500 MB | Node.js dev-сервер |
...
| **Итого (без Ollama)** | **18–28 ядер** | **~11 GB** | **~27 GB** | **~37+ GB** | |
...
|-----------|----------------|--------|----------|-----------|
| 5173 | 5173 | Frontend | HTTP | Web-интерфейс пользователя |
...
|----------|-----------|------|----------|-----------|
| Frontend | FastAPI | 8000 | HTTP | API-запросы, SSE-стриминг |
...
```
Пользователь ──► Frontend ──► POST /documents/upload (multipart)
                                        │
                                        ▼
                               ┌─────────────────┐
                               │   FastAPI        │
...
```
Пользователь ──► Frontend ──► POST /chat/stream (SSE)
                                        │
                                        ▼
                               ┌─────────────────────────────────────────────────┐
                               │              LangGraph Agent                    │
...
```
Пользователь ──► Frontend ──► POST /feedback
                                    │
                                    ▼
...
4. Frontend автоматически обновляет токен при получении HTTP 401
...
| Node modules (frontend) | Frontend | Docker volume: `frontend_node_modules` | N/A |
...
|---|-----------|-----------|----------|------|---------|
| 1 | Docker-образы | `registry-1.docker.io`, `ghcr.io`, `production.cloudflare.docker.com` | HTTPS | 443 | Загрузка базовых образов: `python:3.12-slim`, `node:20-alpine`, `postgres:16-alpine`, `mongo:7`, `redis:7-alpine`, `minio/minio:latest`, `minio/mc:latest`, `qdrant/qdrant:latest`, `clickhouse/clickhouse-server`, `langfuse/langfuse:3`, `langfuse/langfuse-worker:3` |
...
| 4 | npm-пакеты | `registry.npmjs.org` | HTTPS | 443 | Установка frontend-зависимостей (~20 пакетов: React, Vite, Tailwind и др.) |
| 5 | NLTK Data | `raw.githubusercontent.com` | HTTPS | 443 | Загрузка токенизаторов: `averaged_perceptron_tagger_eng`, `punkt_tab` |
...
| 7 | Ollama модели | `ollama.com`, `registry.ollama.ai` | HTTPS | 443 | Команда `ollama pull` для загрузки моделей: `nomic-embed-text:latest` (~274 MB), `llama3.1` (~4.7 GB) или другая LLM |
...
|---|-------------|---------|
| 9 | **Закрыть неиспользуемые порты** | Оставить открытыми для внешнего доступа только порты 5173 (frontend) и 8000 (API). Все остальные порты (Qdrant, Redis, MongoDB, PostgreSQL, MinIO, Model Server) убрать из `ports:` в docker-compose и оставить только внутренний доступ через Docker-сеть |
| 10 | **HTTPS/TLS** | Настроить reverse proxy (nginx/traefik) с TLS-сертификатами для портов 5173 и 8000 |
| 11 | **Сетевая сегментация** | Выделить отдельные Docker-сети для: frontend, backend, data (БД), monitoring (Langfuse) |
...
| 19 | **Frontend production build** | Заменить Vite dev-server на nginx с собранными статическими файлами |
| 20 | **SSL verification** | После настройки внутренних CA-сертификатов убрать `PYTHONHTTPSVERIFY=0` и `NODE_TLS_REJECT_UNAUTHORIZED=0`, заменив на доверенные корневые сертификаты |
